...
type Product = {
  id: number; barcode: string | null; name: string; category: string; unit: string;
  price: number; mrp: number; emoji: string; stock: number; active: boolean;
};
type ImportRow = {
...
const blank = { barcode: "", name: "", category: "Fresh", unit: "1 pc", price: "", mrp: "", emoji: "🛍️", stock: "10" };
...
export default function ProductManager() {
  ...
  const submit = async (e: FormEvent) => {
    e.preventDefault(); const r = await authFetch("/api/products", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...form, price: Number(form.price), mrp: Number(form.mrp), stock: Number(form.stock) }) });
    const d = await r.json(); if (!r.ok) return setNote(d.error); setForm(blank); setNote("Product added successfully"); void load();
  };
  const findBarcode = (e: FormEvent) => {
    e.preventDefault(); const code = scan.trim(); if (!code) return;
    const found = products.find((p) => p.barcode === code);
    if (found) { setNote(`${found.name} found • Stock ${found.stock} • ₹${found.price}`); document.getElementById(`product-${found.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }); }
    else { setForm({ ...blank, barcode: code }); setNote("New barcode. Complete the product details below."); }
  ...
  const remove = async (id: number) => { await authFetch(`/api/products?id=${id}`, { method: "DELETE" }); setNote("Product removed from storefront"); void load(); };
  return <main className="manager-page">
    ...
    <section>
      ...
      <div className="manager-grid">
        <form className="manager-form" onSubmit={submit}><h2>Add one product</h2>
          ...
          <div className="form-row"><label>Category<input required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}/></label><label>Emoji<input value={form.emoji} onChange={(e) => setForm({ ...form, emoji: e.target.value })}/></label></div>
          ...
          <div className="form-row"><label>Selling price ₹<input required type="number" min="0.01" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })}/></label><label>MRP ₹<input required type="number" min="0.01" step="0.01" value={form.mrp} onChange={(e) => setForm({ ...form, mrp: e.target.value })}/></label></div><button>Add product →</button>
        </form>
        <div className="catalogue-list"><h2>Current products <span>{products.length}</span></h2>{products.map((p) => <article id={`product-${p.id}`} key={p.id}><i>{p.emoji}</i><div><b>{p.name}</b><small>{p.category} • {p.unit} • Stock {p.stock}{p.barcode ? ` • ${p.barcode}` : " • No barcode"}</small></div><strong>₹{p.price}</strong><button onClick={() => remove(p.id)}>Remove</button></article>)}</div>
      </div>
