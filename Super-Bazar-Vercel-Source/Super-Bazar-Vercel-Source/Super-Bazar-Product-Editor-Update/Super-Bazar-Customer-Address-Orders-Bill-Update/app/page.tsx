...
const emptyAddress = { label: "Home", recipientName: "", phone: "", line1: "", line2: "", city: "Bengaluru", state: "Karnataka", postalCode: "", isDefault: true };
const formatAddress = (a: Address) => [a.line1, a.line2, a.city, a.state, a.postalCode].filter(Boolean).join(", ");
const categories = [
  ["All", "🛍️", "All products"],
...
export default function Home() {
  ...
  const [location, setLocation] = useState("Select delivery address");
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (user) setUserName(user.user_metadata?.full_name || user.email?.split("@")[0] || "Customer");
    });
    authFetch("/api/addresses").then(async (r) => r.ok ? r.json() : { addresses: [] }).then((d) => {
      const rows: Address[] = d.addresses || [];
      setAddresses(rows);
      if (rows[0]) { setSelectedAddress(rows[0]); setLocation(formatAddress(rows[0])); }
    }).catch(() => {});
  }, []);
  const chooseAddress = (address: Address) => {
    ...
    flash("Delivery address selected");
  };
  const saveAddress = async () => {
    ...
    try {
      ...
      setAddresses((rows) => [data.address, ...rows.map((a) => ({ ...a, isDefault: data.address.isDefault ? false : a.isDefault }))]);
      setAddressDraft(emptyAddress);
      chooseAddress(data.address);
    } catch (error) { flash(error instanceof Error ? error.message : "Could not save address"); }
            ...
            {shown.map((p) => (
              <article className="product" key={p.id}>
                <div className="product-image">
                  {p.imageUrl ? <Image src={p.imageUrl} alt={p.name} fill sizes="(max-width: 560px) 45vw, (max-width: 900px) 30vw, 16vw"/> : <span>{p.emoji}</span>}
                  <small>{p.badge}</small>
                  <button
            ...
            {count === 0 ? (
              ...
              <>
                ...
                <div className="cart-items">
                  {cartItems.map((p) => (
                    <article key={p.id}>
                      <div className="cart-pic">{p.imageUrl ? <Image src={p.imageUrl} alt={p.name} width={48} height={48}/> : p.emoji}</div>
                      <div>
                ...
                <button className="checkout-btn" onClick={() => {
                  if (!selectedAddress) { setAddressOpen(true); flash("Add your delivery address first"); }
                  else setCheckout(1);
                }}>
                  <span>
      ...
      {addressOpen && (
        <div className="modal-wrap">
          ...
          <section className="modal address-modal">
            ...
            {!addressForm && <>
              {!addresses.length && <p className="address-empty">No saved address yet. Add your home, hostel or work address.</p>}
              {addresses.map((address) => <button className={`address-choice${selectedAddress?.id === address.id ? " active" : ""}`} key={address.id} onClick={() => chooseAddress(address)}>
                <span>{address.label.toLowerCase().includes("work") ? "🏢" : "🏠"}</span>
                <div><b>{address.label} • {address.recipientName}</b><small>{formatAddress(address)} • {address.phone}</small></div>
                <i>{selectedAddress?.id === address.id ? "✓" : "→"}</i>
              </button>)}
              <button className="new-address" onClick={() => setAddressForm(true)}>＋ Add a new address</button>
            </>}
            {addressForm && <div className="address-form">
              <div className="form-row"><label>Label<input value={addressDraft.label} onChange={(e) => setAddressDraft({ ...addressDraft, label: e.target.value })} placeholder="Home / Hostel / Work" /></label><label>Full name<input value={addressDraft.recipientName} onChange={(e) => setAddressDraft({ ...addressDraft, recipientName: e.target.value })} /></label></div>
              <label>Mobile number<input inputMode="tel" value={addressDraft.phone} onChange={(e) => setAddressDraft({ ...addressDraft, phone: e.target.value })} placeholder="10-digit mobile number" /></label>
              <label>Flat / house / building / street<input value={addressDraft.line1} onChange={(e) => setAddressDraft({ ...addressDraft, line1: e.target.value })} /></label>
              <label>Area / landmark (optional)<input value={addressDraft.line2} onChange={(e) => setAddressDraft({ ...addressDraft, line2: e.target.value })} /></label>
              <div className="form-row"><label>City<input value={addressDraft.city} onChange={(e) => setAddressDraft({ ...addressDraft, city: e.target.value })} /></label><label>PIN code<input inputMode="numeric" maxLength={6} value={addressDraft.postalCode} onChange={(e) => setAddressDraft({ ...addressDraft, postalCode: e.target.value })} /></label></div>
              <label>State<input value={addressDraft.state} onChange={(e) => setAddressDraft({ ...addressDraft, state: e.target.value })} /></label>
              <div className="address-actions"><button onClick={() => setAddressForm(false)}>Cancel</button><button className="primary" disabled={addressBusy} onClick={saveAddress}>{addressBusy ? "Saving…" : "Save address"}</button></div>
            </div>}
          </section>
        </div>
      )}
          ...
          <section className="modal checkout-modal">
            ...
            {checkout === 1 && (
              <>
                ...
                <button className="address-choice active">
                  <span>⌖</span>
                  <div>
                    <b>{selectedAddress?.label} • {selectedAddress?.recipientName}</b>
                    <small>{location} • {selectedAddress?.phone}</small>
                  </div>
                  <i>✓</i>
                </button>
                <label className="instructions">
                  Delivery instructions
                  <textarea value={deliveryInstructions} onChange={(e) => setDeliveryInstructions(e.target.value)} placeholder="e.g. Leave at the security desk" />
                </label>
            ...
            {checkout === 3 && (
              <div className="success">
                ...
                <p>
                  Order <b>#SB{String(orderId || 0).padStart(4, "0")}</b> will
                ...
                <div className="success-actions"><a className="primary" href="/orders">View order & bill</a><button onClick={() => setCheckout(0)}>Continue shopping</button></div>
              </div>
            )}
          </section>
        </div>
